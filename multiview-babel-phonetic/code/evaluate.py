...
if __name__ == "__main__":
    ...
    subwords_to_vectors = data.get_subwords_to_vectors(my_config.feature_fn)
    ...
    for lang_id in range(n_lang):
        this_set = data.DevDataset(feats=my_config.feats[lang_id],
                                   ...
                                   stack_frames=True,
                                   batch_size=500,
                                   subwords_to_vectors= subwords_to_vectors)
        datasets.append(this_set)

    net = net.MultiViewRNN_Phonetic(config=config,
                                    feat_dim=datasets[0].feat_dim,
                                    phone_feat_dim=datasets[0].phone_feat_dim,
                                    loss_fun=None,
                                    use_gpu=True)
    ...
    for subword in this_subwords:
        if subword in ['<noise>', '<hes>', '<v-noise>']:
            continue
        feature_vec = torch.from_numpy(subwords_to_vectors[subword]).type(torch.float)

        with torch.no_grad():
            emb = linear_net(feature_vec).detach().cpu().numpy()
